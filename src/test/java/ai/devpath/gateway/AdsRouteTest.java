...
class AdsRouteTest {
  ...
  @Test
  void adsRequireJwt() {
    web.get().uri("/ads?slot=DASHBOARD_TOP").exchange()
        .expectStatus().isUnauthorized();
  }

  @Test
  void authenticatedAdsRequestMatchesRoute() {
    web.get().uri("/ads?slot=DASHBOARD_TOP")
        .header(HttpHeaders.AUTHORIZATION, "Bearer test-token")
        .exchange()
        .expectStatus().value(AdsRouteTest::assertGatewayMatchedRoute);
  }
