# CLAUDE.md — devpath-gateway

> API Gateway — 모든 클라이언트 트래픽의 단일 진입점 (OAuth2 + JWT)
...
- 패키지: `ai.devpath.gateway` / 메인: `GatewayApplication`
- reactor-test 사용 (WebFlux). 라우팅 변경은 통합 테스트로 검증한다.
